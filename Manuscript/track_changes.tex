...
%DIF ADD main.tex               Thu Jun  1 23:49:11 2023
...
Indeed, the major axis of phenotypic change among species \DIFaddbegin \DIFadd{beak shapes }\DIFaddend at megaevolutionary scales is an emergent property of innovation across clades. Our analyses suggest that the reorientation of phenotypes via innovation is a ubiquitous route for divergence that can arise through gradual change alone, opening up new avenues for evolution to explore.
...
i) idiosyncratic species-level innovationagainst clade-level innovation (i.
e. reorientation of }\DIFdelend \DIFaddbegin \DIFadd{among closely related species.
...
Both innovation and elaboration could in principle occur at }\DIFaddend the \DIFdelbegin \DIFdel{directions of phenotypic variation) ,
and ii) evolution along versus away from major axes of phenotypic variation .
The contributions of these multivariate, evolutionary pathways are poorly-known despite their importance for understanding the origins of diversity from macro- to }\DIFdelend \DIFaddbegin \DIFadd{macro- or }\DIFaddend mega-evolutionary \DIFdelbegin \DIFdel{scales.
Here we estimate for the first time, using novel statistical methods, the relative contribution of elaboration }\DIFdelend \DIFaddbegin \DIFadd{scale.
...
For clarity throughout the manuscript, we refer to elaboration and innovation as the general concept described above but use the following terms to distinguish macroevolutionary scale (species) and megaevolutionary scale (clade) elaboration and innovation: $elaboration_{species}$, $innovation_{species}$, $elaboration_{clade}$ }\DIFaddend and \DIFdelbegin \DIFdel{innovation to the remarkable global radiation and diversity of birds beaks.}\DIFdelend \DIFaddbegin \DIFadd{$innovation_{clade}$.
...
\noindent \textbf{\DIFadd{Figure \ref{Fig:cheatsheet}}}
\DIFadd{Relationships between key figures in the main text.
...
Addressing the relative contributions of elaboration and innovation to the origins of biodiversity in deep-time requires\DIFaddbegin \DIFadd{: }\DIFaddend 1) large, multivariate datasets to allow exploration of trait covariances at different scales, 2) reliable and efficient computational methods to estimate the major axes of phenotypic variation, and 3) a set of mathematical tools that can estimate degrees of elaboration and innovation at any scale.
...
We regard the major axis of \DIFdelbegin \DIFdel{phenotypic variation }\DIFdelend \DIFaddbegin \DIFadd{the evolutionary rate matrix }\DIFaddend as a deep-time analogue of the microevolutionary \textbf{G} matrix \cite{robinson2013quantifying} and posit that it represents the \DIFaddbegin \DIFadd{among species }\DIFaddend line of least evolutionary resistance, capturing the effects of historical contingency on multivariate evolution.
...
We tested the expectation, derived from adaptive radiation theory, that species divergence is biased along \DIFdelbegin \DIFdel{evolutionary }\DIFdelend \DIFaddbegin \DIFadd{phylogenetic }\DIFaddend lines of least resistance by calculating species-specific measures of \DIFdelbegin \DIFdel{innovation and elaboration.
...
We found \DIFaddbegin \DIFadd{typically }\DIFaddend higher values of \DIFdelbegin \DIFdel{elaboration than innovation overall across all levels (Fig\ref{Fig:phylogeny}) and globally more elaboration and innovation at the global phylogenetic and super-order levels compared to the order level.
}\DIFdelend \DIFaddbegin \DIFadd{$elaboration_{species}$ than $innovation_{species}$ (Fig. \ref{Fig:phylogeny}) %DIF > @L_rephrase_wefound
and strong clustering of both $elaboration_{species}$ and $innovation_{species}$ (median $elaboration_{species}$ for all orders = 0.114, 95\%CI = 0.004-0.485;  $elaboration_{species}$ Pagel's $\lambda$ = 0.888; median $innovation_{species}$ for all orders = 0.071, 95\%CI = 0.018-0.224); $innovation_{species}$ Pagel's $\lambda$ = 0.848).%DIF > @L_ei_species_values. 
...
\noindent \textbf{\DIFadd{Figure \ref{Fig:phylogeny}}}
\DIFadd{Avian phylogeny (n = 8748 species) showing Euclidean distance of species to the centroid of beak space (branches, cividis scale), and distributions of species beak shape $elaboration_{species}$ (inner circle, orange scale), and $innovation_{species}$ (outer circle, blue scale).
...
\DIFaddend Phylogenetic distributions of \DIFdelbegin \DIFdel{innovation and elaboration }\DIFdelend \DIFaddbegin \DIFadd{$elaboration_{species}$ and $innovation_{species}$ }\DIFaddend broadly hold whether comparisons are made at the order, super-order \DIFdelbegin \DIFdel{, or global }\DIFdelend \DIFaddbegin \DIFadd{($elaboration_{species}$ Pagel's $\lambda$ = 0.853; $innovation_{species}$ Pagel's $\lambda$ = 0.914), or class-wide }\DIFaddend phylogenetic level (\DIFdelbegin \DIFdel{Fig. \ref{Fig:phylogeny})%DIF < {#fig_correlations}),%TODO: maybe add to supp?
 }\DIFdelend \DIFaddbegin \DIFadd{$elaboration_{species}$ Pagel's $\lambda$ = 0.935; $innovation_{species}$ Pagel’s $\lambda$ = 0.837; Fig. \ref{Fig:phylogeny_supplement}); \ref{Fig:figure_correlations} %DIF > @L_correlation_figure_link)
...
However, we observe various combinations of elaboration and innovation across birds. Examples of high innovation and}\DIFdelend \DIFaddbegin \DIFadd{, it is also commonly positively correlated with $innovation_{species}$ (Fig. \ref{fable_correlations} \ref{fable_correlations_supplementary}. %DIF > @L_correlation_figure_link).
...
\noindent \textbf{Figure \DIFdelbegin \DIFdel{\ref{Fig:phylogeny}:}\DIFdelend \DIFaddbegin \DIFadd{\ref{fable_correlations}}\DIFaddend }
\DIFdelbegin \DIFdel{The avian phylogeny (n = 8748 species) coloured by species beak shape innovation and elaboration level.
...
Innovation and elaboration scores are represented concentrically in a nested fashion with the most external pairs of circles (innovation and elaboration bands) representing comparisons for each species at the order level, then at the }\DIFdelend \DIFaddbegin \DIFadd{Posterior correlations between $elaboration_{species}$ and $innovation_{species}$ for each }\DIFaddend super-order\DIFdelbegin \DIFdel{level, and finally at the global phylogenetic level. The boxplots at the bottom of the figure summarise the distribution of the median innovation and elaboration values for all levels (bottom left), showing that elaboration is more common than elaboration,
...
are the result of innovation we would expect to see changes in the orientation }\DIFaddend of trait space \DIFdelbegin \DIFdel{at deeper phylogenetic scales with greater distances generally composed of more elaboration than innovation.A companion plot showing further nested structure within the Passeriformes is shown in Fig. S\ref{fig_phylogeny_passeriformes}. 
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{To investigate the conflict implied by our observations of species-level elaboration and previously reported deep-time jumps in phenotype, we measured the elaboration and innovation of different clades.
To do thiswe translated the major axes of phenotypic }\DIFdelend \DIFaddbegin \DIFadd{of clades (i.e. orders or super-orders) relative to the phylogenetic major axis of beak variation of the whole class Aves.
To test this, we measured the $elaboration_{clade}$ and $innovation_{clade}$ by translating the phylogenetic major axis of beak }\DIFaddend variation of each clade's (i.e. super-orders and orders) \DIFdelbegin \DIFdel{phylogenetic variance-covariance }\DIFdelend \DIFaddbegin \DIFadd{evolutionary rate }\DIFaddend matrix onto the \DIFdelbegin \DIFdel{global }\DIFdelend \DIFaddbegin \DIFadd{class-wide }\DIFaddend phylogenetic major axis of \DIFdelbegin \DIFdel{phenotypic }\DIFdelend \DIFaddbegin \DIFadd{beak }\DIFaddend variation so that they shared the same origin in the shapespace.
...
This differs from previous inference on a subset of our data that implied generally consistent and low dimensionality within clades \mbox{%DIFAUXCMD
\cite{cooney2017mega}}\hskip0pt%DIFAUXCMD
.
This suggests that there is no deep-time analogue of the genetic line of least resistance and that across scales there is remarkable flexibility in the routes to innovation, consistent with the idea that morphological divergence may be less constrained in deep-time than is often assumed \mbox{%DIFAUXCMD
\cite{venditti2011multiple}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend 

\begin{figure}[!htbp]
\centering
   \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\textwidth]{Figures/ellipses.pdf}
%DIFDELCMD < %%%
...
\DIFdelbegin \DIFdel{Panel A) shows ellipses representing }\DIFdelend \DIFaddbegin \DIFadd{$Elaboration_{clade}$ and $innovation_{clade}$ at the megaevolutionary scale for each order and super-order in the bird phylogeny.
The black and gray ellipses are }\DIFaddend the scaled average \DIFdelbegin \DIFdel{posterior variance-covariance response }\DIFdelend \DIFaddbegin \DIFadd{evolutionary rate matrices }\DIFaddend from the pGLMM models for \DIFaddbegin \DIFadd{respectively the clades (black) and the class-wide phylogeny (gray).
The ellipses are centered on the position of the clade in the shapespace.
The associated black bar plots represent the variance on each of the eight dimensions in the shapespace with the top gray bar plot representing the variance on }\DIFaddend each \DIFdelbegin \DIFdel{super-order (coloured ellipses) compared to the global (all birds) phylogenetic component of the models (grey ellipses).
...
Inset barplots show the proportion of variance associated with each }\DIFdelend of the eight \DIFdelbegin \DIFdel{PC axes in shape morphospace.
The inset boxplots correspond to the elaboration (E) and innovation (I}\DIFdelend \DIFaddbegin \DIFadd{dimensions for the class-wide phylogeny.
The orange and blue distributions represent respectively the distribution of the $elaboration_{clade}$ (orange) and the $innovation_{clade}$ (blue}\DIFaddend ) scores for \DIFdelbegin \DIFdel{all 4000 posterior samples.
The dots represent the median elaboration and innovation values while the thick and dashed lines represent }\DIFdelend \DIFaddbegin \DIFadd{each clade and the dots are the median $elaboration_{clade}$ or $innovation_{clade}$ and }\DIFaddend the \DIFaddbegin \DIFadd{solid and the dashed lines representing the }\DIFaddend 50\% and \DIFaddbegin \DIFadd{the }\DIFaddend 95\% confidence \DIFdelbegin \DIFdel{intervals respectively.
These scores were calculated on the unscaled ellipses resulting in different scales }\DIFdelend \DIFaddbegin \DIFadd{intervales of the distribution of the scores across the 4000 posterior samples.
The ticks on the y-axis always represent five arbitrary units }\DIFaddend of elaboration and innovation\DIFdelbegin \DIFdel{for each plot.
Panel B) shows the results for each order. A }\DIFdelend \DIFaddbegin \DIFadd{.
An alternative visualization of the same information is available in the supplementary materials in Fig. \ref{Fig:ellipses_rainbow} along with a }\DIFaddend companion plot showing further nested structure within the Passeriformes \DIFdelbegin \DIFdel{is shown }\DIFdelend in Fig. \DIFdelbegin \DIFdel{S}\DIFdelend \ref{fig_ellipses_passeriformes}.
...
\centering
   \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.9\textwidth]{Figures/orthogonality_results.pdf}
%DIFDELCMD < %%%
...
\DIFdelbegin \textbf{\DIFdel{Clade's orthogonality.}}
%DIFAUXCMD
\DIFdel{Orthogonality of }\DIFdelend \DIFaddbegin \DIFadd{Amount of orthogonality of }\DIFaddend each clade's \DIFaddbegin \DIFadd{phylogenetic }\DIFaddend major axis of \DIFdelbegin \DIFdel{phenotypic }\DIFdelend \DIFaddbegin \DIFadd{beak }\DIFaddend variation compared to their parent or parent's parent clade.
\DIFdelbegin \DIFdel{Orthogonality }\DIFdelend \DIFaddbegin \DIFadd{The amount of orthogonality }\DIFaddend is represented on the horizontal axis and scales from 0 (modulo of $0^\circ$) to 1 (modulo of $90^\circ$) with the background \DIFdelbegin \DIFdel{grey and dashed grey }\DIFdelend \DIFaddbegin \DIFadd{gray and dashed gray }\DIFaddend lines representing, respectively, an orthogonality of 0.5 (modulo of $45^\circ$) and 0.75 (modulo of $67.5^\circ$).
Dots represent the median orthogonality of each clade and the lines their 95\% \DIFdelbegin \DIFdel{CI}\DIFdelend \DIFaddbegin \DIFadd{confidence intervals (CI)}\DIFaddend .
No super-order's \DIFaddbegin \DIFadd{phylogenetic }\DIFaddend major axis of \DIFdelbegin \DIFdel{phenotypic }\DIFdelend \DIFaddbegin \DIFadd{beak }\DIFaddend variation is parallel to the \DIFdelbegin \DIFdel{global }\DIFdelend \DIFaddbegin \DIFadd{class-wide }\DIFaddend phylogenetic major axis \DIFdelbegin \DIFdel{(red }\DIFdelend \DIFaddbegin \DIFadd{of beak variation (purple }\DIFaddend lines and circles) and \DIFdelbegin \DIFdel{12}\DIFdelend \DIFaddbegin \DIFadd{22}\DIFaddend /\DIFdelbegin \DIFdel{25 orders are }\DIFdelend \DIFaddbegin \DIFadd{27 orders are on average (median) }\DIFaddend at least half orthogonal \DIFdelbegin \DIFdel{to the }\DIFdelend \DIFaddbegin \DIFadd{(>  $45^\circ$) to the class-wide phylogenetic }\DIFaddend major axis of \DIFdelbegin \DIFdel{their super-order (green lines and circles)}\DIFdelend \DIFaddbegin \DIFadd{beak variation}\DIFaddend .
These results are \DIFdelbegin \DIFdel{even clearer when compared to the global phylogenetic major axis (green dashed lines and open circles) where only 3}\DIFdelend \DIFaddbegin \DIFadd{to be contrasted with the orientation of each order relative to their super-order where on average (median) only 14}\DIFaddend /\DIFdelbegin \DIFdel{25 are less than half orthogonal }\DIFdelend \DIFaddbegin \DIFadd{27 are at least half orthogonal to the orientation of their super-orders’s phylogenetic major axis of beak variation}\DIFaddend .
We also indicate the number of species (n) and the standard deviation (sd) of \DIFdelbegin \DIFdel{their major axes of phenotypic variation orientation }\DIFdelend \DIFaddbegin \DIFadd{the orientation of their phylogenetic major axis of beak variation }\DIFaddend over the 4000 variance-covariance posteriors (sd; expressed in degrees).
\DIFaddbegin \DIFadd{Note that noclades are clearly parallel to their parent’s or parent’s parent’s ellipses (i.e.
no 95\% CI includes 0).
}\DIFaddend For each clade we also measured the posterior probability of each clade's orientation being different from their parent's clade or the \DIFdelbegin \DIFdel{global }\DIFdelend \DIFaddbegin \DIFadd{class-wide }\DIFaddend phylogenetic major axis \DIFaddbegin \DIFadd{of beak variation }\DIFaddend relative to their sample size and sd.
The stars represent the posterior probability of the clade's orientation being different from the comparison clade \DIFdelbegin \DIFdel{. }\DIFdelend (*** = pp $> 0.99$; ** = pp $>0.95$; * = pp $> 0.9$; . = pp $> 0.8$).
Only \DIFdelbegin \DIFdel{two out of six super-orders; }\DIFdelend \DIFaddbegin \DIFadd{1/}\DIFaddend 6 \DIFaddbegin \DIFadd{super-orders, 8}\DIFaddend /\DIFdelbegin \DIFdel{25 }\DIFdelend \DIFaddbegin \DIFadd{27 }\DIFaddend orders relative to \DIFdelbegin \DIFdel{their super-order and 9}\DIFdelend \DIFaddbegin \DIFadd{class-wide phylogeny and 6}\DIFaddend /\DIFdelbegin \DIFdel{25 orders relative to the phylogeny }\DIFdelend \DIFaddbegin \DIFadd{27 relative to their super-orders }\DIFaddend have a pp $> 0.99$.
This is the result of the variation in sample size (n) and standard variation (sd) among clades.
A companion plot showing further nested structure within the Passeriformes is shown in Fig. \DIFdelbegin \DIFdel{S}\DIFdelend \ref{fig_orthogonality_passeriformes}.

\DIFdelbegin %DIFDELCMD < \bigskip
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Evolutionary innovation }\DIFdelend \DIFaddbegin \DIFadd{Substantial variation in $elaboration_{clade}$ and $innovation_{clade}$ arises among clades including clades with relatively low $elaboration_{clade}$ and $innovation_{clade}$(e.g. Cuculiformes; Fig. \ref{Fig:ellipses}), low $elaboration_{clade}$ and high $innovation_{clade}$ (e.g. Galliformes; Fig. \ref{Fig:ellipses}), high $elaboration_{clade}$ and low $innovation_{clade}$ (e.g. Coraciiformes; Fig. \ref{Fig:ellipses}), and both relatively high $elaboration_{clade}$ and $innovation_{clade}$ (e.g. Podicipediformes; Fig. \ref{Fig:ellipses}).
...
Evolutionary theory predicts that lines of least resistance should break down with time since divergence \mbox{%DIFAUXCMD
...
\DIFadd{Our observations of heterogeneity in the orientation of the phylogenetic major axis of beak variation among clades is further supported by consistent evidence for high amounts of orthogonality of clades relative to the class-wide phylogenetic major axis of beak variation (Fig. \ref{Fig:ellipses} - Fig. \ref{Fig:orthogonality}).
...
This differs from previous inference from a subset of our data analysed  without incorporating phylogeny, that implied generally consistent and low dimensionality within clades \mbox{%DIFAUXCMD
\cite{cooney2017mega}}\hskip0pt%DIFAUXCMD
.
...
Across scales there is remarkable flexibility in the routes to innovation, consistent with the idea that morphological divergence may be less constrained in deep-time than is sometimes assumed \mbox{%DIFAUXCMD
\cite{venditti2011multiple}}\hskip0pt%DIFAUXCMD
. %DIF >  @L_occasionaly 
}

\DIFadd{Evolutionary $innovation_{clade}$ }\DIFaddend can arise in \DIFdelbegin \DIFdel{any direction }\DIFdelend \DIFaddbegin \DIFadd{many directions }\DIFaddend in trait space \DIFdelbegin \DIFdel{.
...
In addition to the lack of alignment of clade \DIFdelbegin \DIFdel{major axes of phenotypic }\DIFdelend \DIFaddbegin \DIFadd{phylogenetic major axis of beak }\DIFaddend variation with the \DIFaddbegin \DIFadd{class-wide }\DIFaddend phylogenetic major axis \DIFaddbegin \DIFadd{of beak variation for birds}\DIFaddend , we also found that within clades, \DIFdelbegin \DIFdel{phenotypic }\DIFdelend \DIFaddbegin \DIFadd{beak shape }\DIFaddend variation is either highly constrained (varying almost entirely along a single axis; e.g. in Galliformes) or higher-dimensional than expected (there is no clear dominant \DIFdelbegin \DIFdel{major axis}\DIFdelend \DIFaddbegin \DIFadd{phylogenetic major axis of beak variation}\DIFaddend ; e.g. in Podicipediformes) from the \DIFdelbegin \DIFdel{global }\DIFdelend \DIFaddbegin \DIFadd{class-wide }\DIFaddend phylogenetic major axis of \DIFdelbegin \DIFdel{phenotypic }\DIFdelend \DIFaddbegin \DIFadd{beak }\DIFaddend variation.
For example, Accipitriformes (hawks and allies) have \DIFaddbegin \DIFadd{phylogenetic }\DIFaddend major axes of \DIFdelbegin \DIFdel{phenotypic }\DIFdelend \DIFaddbegin \DIFadd{beak }\DIFaddend variation that mainly align with the second dimension of the shapespace suggesting distinct directions of \DIFaddbegin \DIFadd{beak }\DIFaddend evolution for the clade relative to the \DIFdelbegin \DIFdel{global }\DIFdelend \DIFaddbegin \DIFadd{class-wide }\DIFaddend phylogenetic level but uniformity in their beak shape within the clade.
...
%DIFDELCMD < %%%
\DIFdelend %DIF > TG: TODO: note in the response letter that we've now nuanced this
...
So, although species \DIFaddbegin \DIFadd{often }\DIFaddend evolve preferentially along a shared \DIFdelbegin \DIFdel{major axis of phenotypic variance, this major axis changes frequently among clades.
...
This idea is similar in concept to multiple adaptive peak models where species evolve in a heterogeneous adaptive landscape and share similar responses to selection pressures within adaptive zones \mbox{%DIFAUXCMD
\cite{hansen1997stabilizing}}\hskip0pt%DIFAUXCMD
.
This implies that gradual directional evolution \mbox{%DIFAUXCMD
\cite{pagel2022general}}\hskip0pt%DIFAUXCMD
, rather than exceptional megaevolutionary jumps \mbox{%DIFAUXCMD
\cite{venditti2011multiple,cooney2017mega}}\hskip0pt%DIFAUXCMD
, may be sufficient to explain diversity in avian beak morphology.
}\DIFdelend \DIFaddbegin \DIFadd{phylogenetic major axes of beak variation within clades, there is frequent deviation from these major axes, and the phylogenetic major axes of beak variation change among clades.
}\DIFaddend 

\begin{figure}[!htbp]
\centering
   \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.9\textwidth]{Figures/relative_EI_gg_style.pdf}
%DIFDELCMD < %%%
...
Comparisons of relative \DIFdelbegin \DIFdel{elaboration and innovation among clades (illustrated in figure \ref{Fig:ellipses}) and among species (figure \ref{Fig:phylogeny}}\DIFdelend \DIFaddbegin \DIFadd{$elaboration_{species}$ and $innovation_{species}$ (Fig. \ref{Fig:phylogeny}) and $elaboration_{clade}$ and $innovation_{clade}$ (Fig. \ref{Fig:ellipses}}\DIFaddend ).
a) the \DIFdelbegin \DIFdel{elaboration (blue}\DIFdelend \DIFaddbegin \DIFadd{$elaboration_{species}$ (purple}\DIFaddend ) for each species in each 4000 posterior samples compared to the \DIFdelbegin \DIFdel{elaboration }\DIFdelend \DIFaddbegin \DIFadd{$elaboration_{clade}$ }\DIFaddend for each of the 35 clades (yellow).
The \DIFdelbegin \DIFdel{scores for each clade }\DIFdelend \DIFaddbegin \DIFadd{$elaboration_{clade}$ scores }\DIFaddend are scaled by the maximum score within the clade (corresponding to the scaled ellipses in \DIFdelbegin \DIFdel{figure }\DIFdelend \DIFaddbegin \DIFadd{Fig. }\DIFaddend \ref{Fig:ellipses}) and the scores for species are scaled by the maximum \DIFdelbegin \DIFdel{species elaboration }\DIFdelend \DIFaddbegin \DIFadd{$elaboration_{species}$ }\DIFaddend score.
This allows us to compare \DIFdelbegin \DIFdel{elaboration in species and clades }\DIFdelend \DIFaddbegin \DIFadd{$elaboration_{species}$ in and $elaboration_{clade}$ }\DIFaddend despite their different sample sizes.
We use the Bhattacharryya \DIFdelbegin \DIFdel{coefficient (BC) }\DIFdelend \DIFaddbegin \DIFadd{Coefficient }\DIFaddend to quantify the overlap of the area under the curve for species and clades.
For elaboration the overlap is \DIFdelbegin \DIFdel{BC }\DIFdelend \DIFaddbegin \DIFadd{Bhattacharryya Coefficient }\DIFaddend = 0.217, indicating no distinguishable dissimilarity in the amount of \DIFdelbegin \DIFdel{elaboration }\DIFdelend \DIFaddbegin \DIFadd{$elaboration_{species}$ and $elaboration_{clade}$ }\DIFaddend across the posterior samples\DIFdelbegin \DIFdel{between species or clades}\DIFdelend .
b) \DIFdelbegin \DIFdel{the same }\DIFdelend as a) but using \DIFdelbegin \DIFdel{innovation score}\DIFdelend \DIFaddbegin \DIFadd{$innovation_{species}$ and $innovation_{clade}$ scores}\DIFaddend .
For innovation the overlap has \DIFdelbegin \DIFdel{BC }\DIFdelend \DIFaddbegin \DIFadd{Bhattacharryya Coefficient }\DIFaddend = 0.039, which indicates a clear difference in the amount of \DIFdelbegin \DIFdel{innovation }\DIFdelend \DIFaddbegin \DIFadd{$innovation_{species}$ and $innovation_{clade}$ }\DIFaddend across the posterior samples\DIFaddbegin \DIFadd{.
}
...
We reach this conclusion because models with interactions }\DIFaddend between species and \DIFdelbegin \DIFdel{clades.
}\DIFdelend \DIFaddbegin \DIFadd{clade metrics provide by far the best model fit overall and removal of terms indicate that the most important interactions are between $innovation_{clade}$  and $innovation_{species}$ followed by $innovation_{clade}$ and $elaboration_{species}$.
...
\begin{tabular}{ c c c }
 \DIFaddFL{So }& \DIFaddFL{many }& \DIFaddFL{stars }\\ 
 \DIFaddFL{*** }& \DIFaddFL{*** }& \DIFaddFL{*** }\\  
 \DIFaddFL{*** }& \DIFaddFL{*** }& \DIFaddFL{*** }\\
\end{tabular}
...
\DIFaddbegin \DIFadd{Taken together, these results suggest that rather than a paradox, the reorientation of trait space can instead arise as a result of species-level innovations within clades arising along common directions in phenotypic space.
This idea is similar in concept to multiple adaptive peak models where species evolve in a heterogeneous adaptive landscape and share similar responses to selection pressures within adaptive zones \mbox{%DIFAUXCMD
\cite{hansen1997stabilizing}}\hskip0pt%DIFAUXCMD
.
This implies that gradual directional evolution \mbox{%DIFAUXCMD
...
The full mathematical procedure is described in detail in the supplementary materials (section \ref{supp_projection} \DIFdelbegin \DIFdel{).}\DIFdelend \DIFaddbegin \DIFadd{- note that the procedure described in the section \ref{supp_projection} is applied to each individual evolutionary rate matrix for each term in the model, e.g. for each 4000 posterior variance matrices individually for each $elaboration_{species}$, $innovation_{species}$, $elaboration_{clade}$ or $innovation_{clade}$ score %DIF > @L_individual matrices).
}\DIFaddend Code to reproduce the analyses is available at \href{https://raw.rawgit.net/TGuillerme/dispRity/master/inst/vignettes/Projection_analysis.html}{this \texttt{dispRity} vignette}%DIF <  TODO: update link to release when releasing
...
because we were interested in measuring the amount of orthogonality between pairs of evolutionary rate matrices rather than simply measuring whether they were different (but see supplementary materials for random skewers correlation results \ref{Fig:random_skewers}). %DIF > @L_noskewers
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \bibliographystyle{naturemag}
%DIFDELCMD < \bibliography{references}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \textbf{\DIFadd{Phylogenetic generalised least squares analysis.}} \DIFadd{We ran phylogenetic generalised least squares models (PGLS) to test the effects of elaboration and innovation on beak shape divergence.
...
%DIFAUXCMD
\DIFdel{We acknowledge IT Services at The University of Sheffield for the provision of the High Performance Computing Service. We thank M. Adams, H. van Grouw, R. Prys-Jones and and Zo\"{e} K. Varley from the Bird Group at the NHM, Tring.
We are indebted to the volunteer citizen scientists at http://www.
markmybird.org for helping to build the database of bird beak shape and contribute to our understanding of avian evolution.
}\textbf{\DIFdel{Funding.}}
%DIFAUXCMD
\DIFdel{This work was funded by the European Research Council (615709 Project ‘ToLERates}\DIFdelend \DIFaddbegin \begin{equation}
...
In addition, we estimated Pagel}\DIFaddend ’\DIFdelbegin \DIFdel{), UKRI-NERC Grant NE/T000139/1 and a Royal Society University Research Fellowship (URF R 180006 to G.H.T.).
}\textbf{\DIFdel{Author contributions.}}
%DIFAUXCMD
\DIFdel{G.H.T., A.P.B., N.C., and T.
G.
designed the study.
J.A.B., C.R. C., E.
C.H., }\DIFdelend \DIFaddbegin \DIFadd{s $\lambda$ for $elaboration_{species}$ }\DIFaddend and \DIFdelbegin \DIFdel{G.H.T. %DIF <  and Z.K.V.
curated the data. T.G. analysed the data.
All authors contributed to the writing of the manuscript.
}\textbf{\DIFdel{Competing interests.}}
%DIFAUXCMD
\DIFdel{We declare no competing interests.
}\DIFdelend \DIFaddbegin \DIFadd{$innovation_{species}$ as univariate traits.
All models were fitted using the phylolm library using a single phylogeny \mbox{%DIFAUXCMD
\cite{phylom}}\hskip0pt%DIFAUXCMD
.
}

\DIFaddend \textbf{\DIFdelbegin \DIFdel{Data an materials availability}\DIFdelend \DIFaddbegin \DIFadd{Reproducibility and repeatability}\DIFaddend .}
The raw and processed data used in this analysis is available from \DIFdelbegin %DIFDELCMD < \href{https://figshare.shef.ac.uk/account/articles/21526146}{%%%
\DIFdelend \DIFaddbegin \href{https://figshare.com/articles/dataset/Innovation_and_elaboration_on_the_avian_tree_of_life/20480355}{\DIFaddend figshare} \cite{fighsaredata}.
The code used to reproduce the figures and tables is available from \href{https://github.com/TGuillerme/elaboration_exploration_bird_beaks}{\DIFdelbegin \DIFdel{GitHub}\DIFdelend \DIFaddbegin \DIFadd{github}\DIFaddend } \cite{githubrepo}.

%DIF <  TODO: Add list of supplementary materials
%DIF <  All this comes after the references
%DIF <  Figures and captions after this
\DIFaddbegin \textbf{\DIFadd{Acknowledgements.}}
\DIFadd{We thank Masahito Tusboi and three other anonymous reviewers for their suggestions that greatly helped improving the manuscript.
}

\textbf{\DIFadd{Funding.}}
\DIFadd{This work was funded by UKRI-NERC Grant NE/T000139/1 and a Royal Society University Research Fellowship (URF R 180006 to GHT).
}
