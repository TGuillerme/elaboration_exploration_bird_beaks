...
#     expect_equal(names(test), unname(MCMCglmm.levels(model_list[[7]])[-1]))
#     test <- analyses.group(axes, base = 3)
#     expect_equal(names(test), unname(MCMCglmm.levels(model_list[[7]])[-3]))
#     test <- analyses.group(axes, base = "animal")
#     expect_equal(names(test), unname(MCMCglmm.levels(model_list[[7]])[-4]))
#     test <- analyses.group(axes, groups = c(1, 2, 3), return = c("angle", "projection"))
#     expect_equal(names(test), unname(MCMCglmm.levels(model_list[[7]])[1:3]))
