...
To do so, for randomly selected posterior variance-covariance matrix we defined $\vec{a}$ as the major axes for the clade of interest and projected it onto $\vec{B}$ as the major overall phylogenetic effect (see above).
...
group_results <- analyses.group(major_axes[1:4], base = "animal")
names(group_results) <- levels(demo_data$clade)
```

The results are reported as boxplots of **log values** for each group indicating which group explores/elaborates more (and to which degree - angle) relatively to each other.

```{r, fig.height = 18, fig.heigh = 6}
## Plot the blob wise metrics
plot.analyses.group(lapply(group_results, log), col = colour_vector)
```
...
And here are the **log values** plotted by group:

```{r}
## Plot results
plot.analyses.tip(lapply(tip_results_global, log), col = colour_vector, group = split(rownames(demo_data), f = demo_data$clade))
```
