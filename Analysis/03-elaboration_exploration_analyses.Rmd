...
covar_matrices <- MCMCglmm.dispRity(
                  data          = demo_data,
...
results_between <- covar.projections.wrapper(covar_matrices,
                                             type = "between",
                                             base = "phylogeny",
                                             dispRity.out = TRUE)
## Plot the results
...
results_phylo <- covar.projections.wrapper(covar_matrices,
                                            type = "within",
                                            base = "phylogeny",
                                            dispRity.out = TRUE)
## Plot the results
...
results_within <- covar.projections.wrapper(covar_matrices,
                                            type = "within",
                                            dispRity.out = TRUE)
## Plot the results
...
```{r, echo = FALSE, eval = TRUE}
library(phytools)
source("~/Packaging/phytools/R/plotTree.wBars.R")

get.disparity.matrix <- function(data, col) {
    ## Get the matrices
    matrices <- lapply(data$disparity, `[[`, 1)
    ## Add the rownames
    list_of_names <- list()
    for(one_group in 1:length(matrices)) {
        rownames(matrices[[one_group]]) <- list_of_names[[one_group]] <- rownames(data$matrix[[1]])[c(data$subsets[[one_group]]$elements)]
    }
    ## Name the groups
    names(list_of_names) <- names(data$subsets)
    ...
    ## Colours
    if(!missing(col)) {
        name.cols <- function(names, col) {
            out <- rep(col, length(names))
            names(out) <- names
            return(out)
        }
        col <- mapply(name.cols, list_of_names, as.list(col))
        names(col) <- NULL
        col <- unlist(col)
    }
    ...
    ## Output
    if(missing(col)) {
        return(list(data = matrix, names = list_of_names))
    } else {
        return(list(data = matrix, names = list_of_names, col = col))
    }
}

## Get the different elements
phylo_elaboration  <- get.disparity.matrix(results_phylo$position,
                                          col = colour_vector[-4])
within_elaboration <- get.disparity.matrix(
    get.subsets(results_within$position, subsets = c(1,2,3)),
                                          col = colour_vector[-4])
phylo_exploration  <- get.disparity.matrix(results_phylo$distance,
                                          col = colour_vector[-4])
within_exploration <- get.disparity.matrix(
    get.subsets(results_within$distance, subsets = c(1,2,3)),
                                          col = colour_vector[-4])
phylo_angle        <- get.disparity.matrix(results_phylo$degree,
                                          col = colour_vector[-4])
within_angle       <- get.disparity.matrix(
    get.subsets(results_within$degree, subsets = c(1,2,3)),
                                          col = colour_vector[-4])
```
