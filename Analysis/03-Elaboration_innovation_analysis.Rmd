...
We can also calculate for each group:

 * It's angle internal variance (as a proxy for internal variance)
 * It's axis significance (Andrew's bit)

### Groups compared to the phylogeny

```{r, eval = FALSE, echo = FALSE}
## Measuring fun
ellipse.stats <- function(data) {
    ## Get the groups list
    groups_list <- unlist(apply(matrix(c(1:(n.subsets(data)-1), rep(n.subsets(data),(n.subsets(data)-1))), ncol = 2), 1, list), recursive = FALSE)

    ## Renanme the phylogeny subset
    names(data$subsets)[n.subsets(data)] <- "phy"

    ## Measure the disparities
    results <- list(
        angles     = dispRity(data, metric = as.covar(projections.between), measure = "degree", between.groups = groups_list),
        alignments = dispRity(data, metric = as.covar(disalignment, VCV = c(FALSE, TRUE), loc = c(TRUE, FALSE)), between.groups = groups_list),
        distances  = dispRity(data, metric = group.dist, probs = c(0.5), between.groups = groups_list)
    )
    return(results)
}
