...
one.plot.correlations <- function(cor.results, col.sub, col.fun = colour.palette, legend.pos = "topleft", pt.cex = 0.5, legend.cex = 0.5, ID, legend_counter) {
    ...
    ## Plot the points
    plot(plot_data$x, plot_data$y, pch = 19, cex = pt.cex, col = plot_data$col, xaxt = "n", yaxt = "n", xlab = "", ylab = "", main = strsplit(cor.results$main, split = ":")[[1]][1])
...
for(i in 1:8) {
    ...
    one.plot.correlations(super_phylo_cor[[i]],col.sub = col_subs[[i]], legend.pos = legends_pos[i], ID = i, pt.cex = 0.5, legend.cex = 2/3, legend_counter = legend_counter)
}
