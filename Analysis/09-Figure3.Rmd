...
one.plot.correlations <- function(cor.results, col.sub, col.fun = colour.palette, legend.pos = "topleft", pt.cex = 0.5, legend.cex = 0.5, ID, ...) {
    ...
    ## Plotting the results
    plot(cor.results$x, cor.results$y, pch = 19, xlab = ifelse(), ylab = ifelse(ID == 1 || ID == 5, "exploration", ""), main = cor.results$main, cex = pt.cex, col = col_vector)
...
par(mfrow = c(2, 4))
lims <- list(x = range(global_data$x),
             y = range(global_data$y))
...
for(i in 1:8) {
    plot.correlations(super_phylo_cor[[i]], xlim = lims$x, ylim = lims$x, tree = NULL, col.sub = col_subs[[i]], legend.pos = legends_pos[i])
}
