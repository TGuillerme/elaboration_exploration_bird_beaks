...
for (i in 1:n) {
    newdatarot[,,i] <- rotonto(x = whole_gpa$mshape, y =new_landmarks[,,i],scale=atts$scale,reflection=atts$reflect,centerweight=atts$centerweight,weights=atts$weights)$yrot
}
...
new_pca_orpda <- stats::predict(original_pca, newdata = proctan_ordat)#array.to(new_proc_tan, to = "matrix"))
new_pca_norop <- stats::predict(original_pca, newdata = proctan_norop)#array.to(new_proc_tan, to = "matrix"))
