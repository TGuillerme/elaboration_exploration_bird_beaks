...
Here we're just creating a mean, median and minimum landmark set using the mean, median and minimum landmark coordinates values.
...
new_spec <- array(data = c(apply(original_landmarks, c(1,2), mean),
                           ...
                           unlist(landvR::select.procrustes(proc_orig$rotated, selector = min))),
                           dim = c(10, 3, 3))
...
new_proc <- Morpho::align2procSym(proc_orig, new_spec)
...
new_pca <- rbind(pca_orig$x, predict(pca_orig, newdata = new_proc_matrix))
...
updated_pca <- update.gpa.pca(whole_gpa, individuals_array)*-1
...
## SOME SMALL SHIFT!
plot(whole_gpa$PCscores[,c(1,2)])
points(updated_pca[-c((dim(whole_gpa$PCscores)[1]+1): dim(updated_pca)[1]), c(1,2)], pch = 19, cex = 0.5)
points(updated_pca[c((dim(whole_gpa$PCscores)[1]+1):dim(updated_pca)[1]), c(1,2)], pch = 19, cex = 0.5, col = "red")
legend("bottomright", pch = c(21, 19, 19), col = c("black", "black", "red"), pt.cex = c(1, 0.5, 1), legend = c("original PCA", "updated PCA", "new specimen"))
```
