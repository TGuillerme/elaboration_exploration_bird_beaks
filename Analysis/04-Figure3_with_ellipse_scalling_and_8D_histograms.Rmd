...
```{r, eval = TRUE, echo = FALSE}
box_size <- 1.5
pdf(file = "../Manuscript/Figures/Figure3_ellipses.pdf", height = box_size*8, width = box_size*10)
...
## Plotting the tree and getting the tip orders
tip_colours_suborders <- plot.short.tree(tree, shapespace, "level1", tip.order = tip_order)
save(tip_colours_suborders, file = "../Data/Processed/tip_colours_suborders.rda")

## Plotting the ellipses
for(i in 1:length(tip_order)) {
    ## Plot each pannel
    wrap.plot.ellipses(i, tip_colours_suborders, suborder_shapespace, results$suborder_results_between)
}
...
## Plotting the tree and getting the tip orders
tip_colours_families <- plot.short.tree(tree, shapespace, "level2", tip.order = tip_order)
save(tip_colours_families, file = "../Data/Processed/tip_colours_families.rda")

## Plotting the ellipses
for(i in 1:length(tip_order)) {
    ## Plot each pannel
    wrap.plot.ellipses(i, tip_colours_families, family_shapespace, results$family_results_between)
}
dev.off()
