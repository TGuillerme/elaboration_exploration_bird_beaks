...
wrap.plot <- function(results, col, main, names, centre.position = TRUE, absolute = TRUE, order = TRUE, xlim = NULL) {
    ...
    if(order) {
        ## Get the ranking
        orders <- do.call(cbind, lapply(results, get.disparity, concatenate = FALSE))
        ## Get the mean values
        orders <- apply(orders, c(1,2), function(x) mean(abs(unlist(x))))
        ## Sum them and rank them
        rank <- order(apply(orders, 1, sum), decreasing = FALSE)
    } else {
    ...
    for(i in 1:n) {
        ...
        one.plot(results[[i]], col = col[rank], centre = is_position && centre.position, main = ifelse(i == centre, main, ""), xlab = names_list[i], zero = FALSE, absolute = absolute, rank = rank, xlim = xlim)
    }
    par(op)
}
...
```{r "before last", fig.height = 16, fig.width = 8, echo = FALSE}
## Plot the results
...
plot.correlations <- function(cor.results, col.sub, col.fun = rainbow, legend.pos = "topleft", pt.cex = 0.5, legend.cex = 0.5, ...) {
    dots <- list(...)
...
for(i in 1:length(super_phylo_cor)) {
    plot.correlations(super_phylo_cor[[i]], col.sub = col_subs[[i]], legend.pos = legends_pos[i])
}
```
...
for(i in 1:length(super_within_cor)) {
    plot.correlations(super_within_cor[[i]], col.sub = col_subs[[i]], legend.pos = legends_pos[i])
}
```
