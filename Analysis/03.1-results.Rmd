...
```{r, echo = FALSE, fig.height = 18, fig.width = 18, eval = TRUE}
par(mfrow = c(2,2))
## Plotting mean axes
covar.plot(superorder_shapespace,
...
```{r, echo = FALSE}
## Plot the results
wrap.plot <- function(results, col, main, names) {

    ## Adding one plot
    one.plot <- function(one_result, xlab, col, zero = TRUE, main, ...) {
        values <- lapply(get.disparity(one_result, concatenate = FALSE), c)
        boxplot(values, horizontal = TRUE, yaxt = "n", col = col, xlab = xlab, main, ...)
        if(zero) {
    ...
    for(i in 1:n) {
        ...
        one.plot(results[[i]], col = col, main = ifelse(i == centre, main, ""), xlab = names_list, zero = (names(results)[[i]] == "position"))
    }
    par(op)
}
...
## Correlations between elaboration and exploration?
...
get.correlations <- function(res, cent.tend = median, exclude.phylo = FALSE, ...) {
    ...
    for(i in 1:n_groups) {
        ...
        plot(centrals_pos[[i]], centrals_dis[[i]], pch = 19, xlab = "elaboration", ylab = "exploration", main = plot_main, cex = 0.5)
    }
    par(op_tmp)
    return(invisible())
}
