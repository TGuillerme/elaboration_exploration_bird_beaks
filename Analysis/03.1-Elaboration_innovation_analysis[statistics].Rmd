...
all_birds_shapespace <- MCMCglmm.subsets(n = 4000, # Just run it on 500 matrices for now
...
## Create the subset for all birds
passeriformes_shapespace <- MCMCglmm.subsets( # Just run it on 500 matrices for now
...
orthogonality <- all_results[, c(1:2, 9:14)]
...
axis(2, at = nrow(plot_table):1, labels = plot_table$Group, las = 2)
axis(1, at = c(0, 0.5, 1), labels = c(0, 0.5, 1))
## Adding the results

for(i in 1:nrow(plot_table)) {
    add.line(plot_table[i, ], counter = i, col = colours[i], total = nrow(plot_table))
    add.stars(plot_table[i, ], counter = i, col = colours[i], total = nrow(plot_table))
}
