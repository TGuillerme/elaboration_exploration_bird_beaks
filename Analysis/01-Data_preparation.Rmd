...
clean.tree <- function(tree, space) {
    matching_tips <- tree$tip.label %in% rownames(space)
    return(drop.tip(tree, tip = tree$tip.label[!matching_tips]))
}
consensus_tree <- clean.tree(consensus_tree, shapespace)

## Reading the distribution of trees
trees_list <- read.tree("../Data/raw/Tree_data/AllBirdsHackett1.tre")
trees_list <- lapply(trees_list, clean.tree, shapespace)
...
form_shape_space_1tree_2levels <- list(
    ...
    "shape_dim"      = shape_axes$dimensions,
    "form_space"     = formspace,
    "form_dim"       = form_axes$dimensions,
