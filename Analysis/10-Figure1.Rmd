...
plot.one.proj.rej <- function(data, name.col, x = TRUE, y = TRUE, main = "") {
    ...
    ## Empty plot
    par(bty = "n")
    plot(NULL, xlim = c(0,1.5), ylim = c(0,lim_up), xaxt = "n", yaxt = "n", xlab = "", ylab = "")
    ...
    ## Add the y axis
    axis(4, at = seq(from = 0, to = lim_up, by = 5), labels = seq(from = 0, to = lim_up, by = 5), las = 2)
...
lay_mat_order <- rbind(
        ...
        matrix(c(46:55), ncol = 10, nrow = 1) # line 7
    )
lay_mat_super <- rbind(
...
# sequence <- c(c(rep(2, 3), 3), c(rep(4, 3), 5), c(rep(6, 3), 7), c(rep(8, 3), 9))

layout_matrix <- layout(rbind(lay_mat_super, (lay_mat_order+17)))
...
for(i in 1:length(tip_order)) {
    ...
    text(0,0.3, names(name.col), cex = 1.5, col = name.col)
