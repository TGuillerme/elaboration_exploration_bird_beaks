...
y axis = levels
column 1 = correlation of e/i projected on the mega level
column 2 = correlation of e/i projected on the macro level
stars for significant correlation between means.

```{r, echo = TRUE, eval = TRUE}
## Get all the correlations for all the clades
cor_mega_level <- get.correlations.scores(results$super_results_phylo)
cor_macro_level <- get.correlations.scores(results$super_results_within)
...
pdf("../Manuscript/Figures/correlations_fable.pdf")
par(mfrow = c(1,2))
par(mar = c(5, 8, 4, 0) + 0.1, bty = "n") # c(bottom, left, top, right)
plot.cor.scores(cor_mega_level, col = super_orders_col,
                y.axis = TRUE, plot.phylo = TRUE,
                main = "Elaboration and innovation\nat the mega level") 
par(mar = c(5, 2, 4, 6) + 0.1, bty = "n") # c(bottom, left, top, right)
plot.cor.scores(cor_macro_level, col = super_orders_col,
