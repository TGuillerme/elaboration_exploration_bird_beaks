...
```{r, eval = TRUE}
## Initialising the results result table
system(paste("echo \"thin, iterations, dimensions, time, d_time, r_time, size, d_size, r_size\" > recording2.csv"))
...
par(mfrow = c(2,1))
plot(NULL, xlim = c(1, 5), ylim = range(run_time), main = "Run time", xlab = "10s of samples (per 10k iterations)", ylab = "log elapsed seconds")
for(i in 1:(nrow(rable)/5)) {
  lines(run_time[(1+5*(i-1)):(5+5*(i-1))], col = cols[i])
}

plot(NULL, xlim = c(1, 5), ylim = range(run_size), main = "Object size", xlab = "10s of samples (per 10k iterations)", ylab = "log bytes")
for(i in 1:(nrow(rable)/5)) {
  lines(run_size[(1+5*(i-1)):(5+5*(i-1))], col = cols[i])
}
```
