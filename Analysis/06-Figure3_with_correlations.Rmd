...
pdf("../Manuscript/Figures/Figure3_b.pdf", height = 4, width = 4)
...
one.plot.correlations <- function(cor.results, col.sub, col.fun = colour.palette, legend.pos = "topleft", pt.cex = 0.5, legend.cex = 0.5, ID, legend_counter, contour = NULL) {
    ...
    ## Adding contours
    if(!is.null(contour)) {
        ## Select the right contour data
        contour_data <- contour[match(rownames(plot_data), names(contour))]        
        ## Calculate the contour
        contour_z <- MASS::kde2d(contour_data/max(contour_data)*max(plot_data$x), contour_data/max(contour_data)*max(plot_data$y), n = 50)

        contour_z <- matrix(0, nrow = length(contour_data), ncol = length(contour_data))
        diag(contour_z) <-  contour_data
        # contour_z <- outer(contour_data/max(contour_data)*max(plot_data$x), contour_data/max(contour_data)*max(plot_data$y))

        ## Adding the contour in the background
        contour(contour_z, lwd = 1, add = TRUE, col = hcl.colors(12, palette = "viridis"), labels = pretty(contour_data))
    }
    ...
    # points(plot_data$x, plot_data$y, pch = 19, cex = pt.cex, col = color.gradient(contour_data))
...
for(i in 1:8) {
    ...
    one.plot.correlations(super_phylo_cor[[i]], col.sub = col_subs[[i]], legend.pos = legends_pos[i], ID = i, pt.cex = 0.5, legend.cex = 2/3, legend_counter = legend_counter, contour = centroids)
}
...
for(i in 1:8) {
    par(mar = c(2,2.5,2,1.5))
    one.plot.correlations(super_group_cor[[i]], col.sub = col_subs[[i]], legend.pos = legends_pos[i], ID = i, pt.cex = 0.5, legend.cex = 2/3, legend_counter = legend_counter)
}
dev.off()
```
