...
> If you use this vignette in published work, please cite @PAPER and @dispRity.

```{r header, echo = TRUE, results = 'hide', message = FALSE, warning = FALSE, eval = TRUE}
...
Mathematically we can use linear algebra to project and reject each species in the trait-space along this major axis of evolution (Figure 4).
...
The first instance, measuring the elaboration and exploration of species within a group, is rather straightforward from [Figure 3](#fig3): the idea is to project each points (defined as the dashed vectors in figure 4) onto a vector of interest (e.g. either the group's own main axis of variance-covariance or the entire phylogeny's one).

We measured the exploration and elaboration using two main approaches (see [Figure 4](#fig4)):
...
Note that the examples depicted in [Figure 4](#fig4) are not scaled for clarity.
In the algorithm implemented in `dispRity`, the results are always effectively scaled as described in [Figure 3](#fig3) and in the [detailed algorithm](#projections-supplementary).
...
covar_data <- MCMCglmm.subsets(data = charadriiformes$data,
                               ...
                               group = MCMCglmm.levels(posteriors)[1:4],
